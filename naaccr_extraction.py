...
def run_extraction_batches(
    ...
    for patient_batch in batch_patients(patient_ids=patient_ids, batch_size=config.batch_size):
        ...
        notes_df = notes_db.select(["PERSON_ID", "MRN", config.note_column, config.date_of_diagnosis_column, config.date_of_diagnosis_column]).where(notes_db.PERSON_ID.isin(patient_batch)).toPandas()
